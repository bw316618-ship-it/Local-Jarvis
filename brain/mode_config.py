...
CREATIVE_PROMPT = (
    ...
    "build_scene_context are the default calls -- they scope automatically "
    "(active document if one is selected, otherwise the whole active "
    "project) and cover the large majority of retrieval needs. Use "
    "build_chapter_ideas_context before proposing chapter directions, and "
    "build_scene_context before writing a scene or chapter. Reach for "
    "search_creative_document or search_creative_project only when you "
    "deliberately need to bypass that automatic scoping -- e.g. checking "
    "the whole project despite a document being selected (search_creative_"
    "project), or confirming something specifically is or isn't in just the "
    "active document (search_creative_document).\n\n"

...
from brain.mode_config import NORMAL, COMPANION, CREATIVE
...
def test_chat_uses_normal_mode_by_default(monkeypatch):
    ...
    jarvis.client.chat.side_effect = fake_chat
...
def test_companion_mode_uses_limited_tools(monkeypatch):
    ...
    def fake_chat(model, messages, tools=None, stream=False, **kwargs):
        ...
        return _stream("That makes sense.")
    ...
    session_state.set_mode(COMPANION)
    jarvis.chat(
        "That was the part I missed most.",
        on_step=lambda m: None,
    )

    assert captured[0][0][0]["content"] == "COMPANION-MODE-PROMPT"
    assert captured[0][1] is llm_module.SESSION_TOOL_SCHEMAS
    assert "Question:\n" not in captured[0][0][-1]["content"]
    assert "User:\nThat was the part I missed most." in captured[0][0][-1]["content"]
...
def test_companion_skips_planning(monkeypatch):
    ...
    monkeypatch.setattr(
        JarvisLLM,
        "_make_plan",
        lambda self, msg: plan_calls.append(msg) or "should not run",
    )
    ...
    jarvis.chat(
        "first this happened, and then something else happened, after that I thought about it",
        on_step=lambda m: None,
    )
...
def test_creative_mode_uses_creative_tools(monkeypatch):
    ...
    def fake_chat(model, messages, tools=None, stream=False, **kwargs):
        captured.append((messages, tools))
        return _stream("I can work from the story.")
...
def test_companion_prompt_policy_is_explicit():
    # The production prompt is replaced in the test fixture, so inspect the
    # actual class initializer without constructing the external client.
    jarvis = JarvisLLM.__new__(JarvisLLM)

    # These are the invariants enforced by the dedicated mode configuration.
